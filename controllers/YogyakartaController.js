import YogyakartaMenu from "../models/YogyakartaModel.js";

export const createYogyakartaMenu = async (req, res) => {
  ...
  try {
    const newMenu = await YogyakartaMenu.create({
      restaurant_name,
      description,
      google_maps_link,
      rating,
...
export const getYogyakartaMenu = async (req, res) => {
  try {
    const menus = await YogyakartaMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteYogyakartaMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await YogyakartaMenu.destroy({
      where: { id },
    });
