import JawaBaratMenu from "../models/JawaBaratModel.js";

export const createJawaBaratMenu = async (req, res) => {
  ...
  try {
    const newMenu = await JawaBaratMenu.create({
      restaurant_name,
      description,
      google_maps_link,
      rating,
...
export const getJawaBaratMenu = async (req, res) => {
  try {
    const menus = await JawaBaratMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteJawaBaratMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await JawaBaratMenu.destroy({
      where: { id },
    });
