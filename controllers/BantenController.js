import BantenMenu from "../models/BantenModel.js";

export const createBantenMenu = async (req, res) => {
  ...
  try {
    const newMenu = await BantenMenu.create({
      restaurant_name,
      description,
      google_maps_link,
      rating,
...
export const getBantenMenu = async (req, res) => {
  try {
    const menus = await BantenMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteBantenMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await BantenMenu.destroy({
      where: { id },
    });
