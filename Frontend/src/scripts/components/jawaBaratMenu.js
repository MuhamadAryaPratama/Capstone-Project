class JawaBaratMenu extends HTMLElement {
  connectedCallback() {
    this.fetchData();
  }
  ...
  async fetchData() {
    try {
      const response = await fetch('http://localhost:4000/jawabarat');
      if (!response.ok) {
  ...
  renderError(error) {
    this.innerHTML = `
          <div class="container">
            <h1>Error Loading Jawa Barat Menu</h1>
...
customElements.define('jawabarat-menu', JawaBaratMenu);
