class SumateraSelatanMenu extends HTMLElement {
  connectedCallback() {
    this.fetchData();
  }
  ...
  async fetchData() {
    try {
      const response = await fetch('http://localhost:4000/sumateraselatan');
      if (!response.ok) {
...
customElements.define('sumateraselatan-menu', SumateraSelatanMenu);
