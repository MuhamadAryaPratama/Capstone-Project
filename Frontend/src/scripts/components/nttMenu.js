class NTTMenu extends HTMLElement {
  connectedCallback() {
    this.fetchData();
  }
  ...
  async fetchData() {
    try {
      const response = await fetch('http://localhost:4000/nusatenggaratimur');
      if (!response.ok) {
  ...
  render(menus) {
    this.innerHTML = `
          <div class="container">
            <h1>Nusa Tenggara Timur Menu</h1>
            <ul>
              ${menus
    .map(
      (menu) => `
  ...
  renderError(error) {
    this.innerHTML = `
          <div class="container">
            <h1>Error Loading Nusa Tenggara Timur Menu</h1>
...
customElements.define('ntt-menu', NTTMenu);
