class SulawesiUtaraMenu extends HTMLElement {
  connectedCallback() {
    this.fetchData();
  }
  ...
  async fetchData() {
    try {
      const response = await fetch('http://localhost:4000/sulawesiutara');
      if (!response.ok) {
  ...
  renderError(error) {
    this.innerHTML = `
          <div class="container">
            <h1>Error Loading Sulawesi Utara Menu</h1>
...
customElements.define('sulawesiutara-menu', SulawesiUtaraMenu);
