class SulawesiBaratMenu extends HTMLElement {
  connectedCallback() {
    this.fetchData();
  }
  ...
  async fetchData() {
    try {
      const response = await fetch('http://localhost:4000/sulawesibarat');
      if (!response.ok) {
...
customElements.define('sulawesibarat-menu', SulawesiBaratMenu);
