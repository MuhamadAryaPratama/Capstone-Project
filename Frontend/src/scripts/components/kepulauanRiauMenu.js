class KepulauanRiauMenu extends HTMLElement {
  connectedCallback() {
    this.fetchData();
  }
  ...
  async fetchData() {
    try {
      const response = await fetch('http://localhost:4000/kepulauanriau');
      if (!response.ok) {
  ...
  renderError(error) {
    this.innerHTML = `
          <div class="container">
            <h1>Error Loading Kepulauan Riau Menu</h1>
...
customElements.define('kepulauanriau-menu', KepulauanRiauMenu);
