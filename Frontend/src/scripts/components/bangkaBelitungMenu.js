class BangkaBelitungMenu extends HTMLElement {
  connectedCallback() {
    this.fetchData();
  }
  ...
  async fetchData() {
    try {
      const response = await fetch('http://localhost:4000/bangkabelitung');
      if (!response.ok) {
  ...
  renderError(error) {
    this.innerHTML = `
          <div class="container">
            <h1>Error Loading Bangka Belitung Menu</h1>
...
customElements.define('bangkabelitung-menu', BangkaBelitungMenu);
