class BaliMenu extends HTMLElement {
  connectedCallback() {
    this.fetchData();
  }
  ...
  async fetchData() {
    try {
      const response = await fetch('http://localhost:4000/bali');
      if (!response.ok) {
...
customElements.define('bali-menu', BaliMenu);
