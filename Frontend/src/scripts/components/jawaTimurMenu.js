class JawaTimurMenu extends HTMLElement {
  connectedCallback() {
    this.fetchData();
  }
  ...
  async fetchData() {
    try {
      const response = await fetch('http://localhost:4000/jawatimur');
      if (!response.ok) {
  ...
  renderError(error) {
    this.innerHTML = `
          <div class="container">
            <h1>Error Loading Jawa Timur Menu</h1>
...
customElements.define('jawatimur-menu', JawaTimurMenu);
