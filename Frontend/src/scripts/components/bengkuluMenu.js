class BengkuluMenu extends HTMLElement {
  connectedCallback() {
    this.fetchData();
  }
  ...
  async fetchData() {
    try {
      const response = await fetch('http://localhost:4000/bengkulu');
      if (!response.ok) {
  ...
  renderError(error) {
    this.innerHTML = `
          <div class="container">
            <h1>Error Loading Bengkulu Menu</h1>
...
customElements.define('bengkulu-menu', BengkuluMenu);
