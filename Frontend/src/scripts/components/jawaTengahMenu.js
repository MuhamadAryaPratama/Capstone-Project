class JawaTengahMenu extends HTMLElement {
  connectedCallback() {
    this.fetchData();
  }
  ...
  async fetchData() {
    try {
      const response = await fetch('http://localhost:4000/jawatengah');
      if (!response.ok) {
  ...
  renderError(error) {
    this.innerHTML = `
          <div class="container">
            <h1>Error Loading Jawa Tengah Menu</h1>
...
customElements.define('jawatengah-menu', JawaTengahMenu);
