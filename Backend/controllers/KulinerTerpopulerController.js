import KulinerTerpopuler from "../models/KulinerTerpopulerModel.js";

export const createKulinerTerpopuler = async (req, res) => {
  ...
  try {
    const newMenu = await KulinerTerpopuler.create({
      restaurant_name,
      description,
      google_maps_link,
      rating,
...
export const getKulinerTerpopuler = async (req, res) => {
  try {
    const menus = await KulinerTerpopuler.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteKulinerTerpopuler = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await KulinerTerpopuler.destroy({
      where: { id },
    });
...
export const editKulinerTerpopuler = async (req, res) => {
  ...
  try {
    ...
    const menu = await KulinerTerpopuler.findByPk(id);
